...
getid <- function(playername)
{players<-read.csv("https://raw.githubusercontent.com/Maiae/NBA-Shot-Charts/master/players.csv", header = TRUE)
  return(players$PLAYER_ID[players$PLAYER_NAME==playername])}
```
...
ggplot(shotDataf, aes(x=LOC_X, y=LOC_Y, xlab("Baseline"), ylab("Sideline"))) +
...
# half court image
img <- readJPEG(system.file("img", "Rlogo.jpg", package="jpeg"))"https://basketballgoalstore.com/wp-content/uploads/half-court.jpg"
court <- grid.raster(jpeg(getURLContent(courtImg.URL)),
